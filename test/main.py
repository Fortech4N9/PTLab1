...
def correct_arguments_string() -> tuple[list[str], str, str]:
    return ["-p", "/home/user/file.txt", "-f", "json"], "/home/user/file.txt", "json"
...
def test_get_path_and_format_from_correct_arguments(
    ...
    path, data_format = get_path_and_format_from_arguments(correct_arguments_string[0])
    assert path == correct_arguments_string[1]
    assert data_format == correct_arguments_string[2]


def test_get_path_with_default_format(
        correct_arguments_string_default_format: tuple[list[str], str, str]) -> None:
    path, data_format = get_path_and_format_from_arguments(correct_arguments_string_default_format[0])
...
def test_incorrect_format_argument(incorrect_format_arguments: list[str]) -> None:
    with pytest.raises(SystemExit):
